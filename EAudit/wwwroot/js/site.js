...
var vmNet = {
    ...
    modal: {
        id: null,
        razorModal: {
            show: function (domId, title) {
                vmNet.modal.id = $(domId);
                console.log($(domId));
                $(domId).modal('show');
                $(domId + ' .modal-title').text(title);
...
function show_error_ajax(status) {
    switch (status) {
        case 404:
            vmNet.notification.show("The page you requested not found.", MSG_ERROR, BLANK_STRING);
            break;
...
function getExt(event, type) {
    ...
    $("#input_nama_file").val(fileName);
    $("#input_tipe_file").val(ext);
